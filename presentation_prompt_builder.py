...
DESIGN_PROMPT = dedent(
    ...
    Apply the following design system in full. Do not skip or shorten it.
    Use it for styling and visual execution only.
    Do NOT let design alter structure, section coverage, wording, numbers, or tables from `report.txt`.
...
PROMPTS = {
    "olympic_holidays": dedent(
        ...
        Your job is to convert this Olympic Holidays report into a polished presentation.

        CRITICAL RULES

        This is NOT a summarisation task.
        This is a STRUCTURAL TRANSFORMATION task.
        ...
        - create a slide for EVERY section in the report
        - convert every data-backed section into a visual slide
        - never invent sections that are not in the report
        - never remove caveats like partial YoY coverage

        SOURCE PRIORITY

        1. `report.txt` is the source of truth
        2. the uploaded `.pptx` shows the intended section order and grouping
        3. the example layout file is for visual style only

        STRUCTURE TO FOLLOW

        The Olympic Holidays report contains these section types:

        1. Executive Summary
        Create 1 slide:
        - title: `Executive Summary`
        - large KPI callouts for core quarter metrics where helpful
        - preserve all summary bullets exactly

        2. Google Trends sections
        There may be multiple separate trend sections, such as:
        - `Cyprus Trends`
        - `Greece Trends`
        - `OH Trends`

        For EACH trends section create 1 slide:
        - title = exact section title
        - line chart from the trend data
        - preserve all trend bullets exactly
        - if the report only provides bullets and not a visible table, still create a trend visual based on the section content and current PPT reference

        3. Auction Insights
        Create 1 slide:
        - title: `Auction Insights`
        - preserve the auction table exactly
        - preserve all auction bullets exactly
        - show the table clearly
        - optionally add a supporting comparison visual, but do NOT remove the table

        4. Overall Performance
        Create 1 slide:
        - title: `Overall Performance`
        - create visuals for the monthly performance trend
        - preserve all bullets exactly

        5. YoY Comparison
        The section title may be:
        - `YoY Quarter Comparison`
        or
        - `YoY Matched-Month Comparison`

        Create 1 slide:
        - preserve the exact title from the report
        - preserve the subtitle/caveat context
        - preserve the full YoY table exactly
        - preserve all bullets exactly
        - if the report states partial matching such as `2 of 3 months matched`, make that caveat visually obvious

        6. End of Period
        Create 1 slide:
        - title: `End of Period`
        - show the closing-month comparison visually
        - preserve all bullets exactly

        7. Channel Performance
        Create 1 slide:
        - title: `Channel Performance`
        - preserve the channel table exactly
        - create visuals for cost share and revenue share
        - preserve all bullets exactly

        8. Generic Performance
        Create 1 slide:
        - title: `Generic Performance`
        - preserve the monthly generic table exactly
        - create trend visuals
        - preserve all bullets exactly

        9. ATC Analysis
        Create 1 slide:
        - title: `ATC Analysis`
        - preserve the ATC table exactly
        - create trend visuals for add-to-cart and cost per ATC
        - preserve all bullets exactly

        VISUAL RULES

        - every time series -> line chart
        - every share/distribution -> pie or donut chart
        - every comparison -> bar chart
        - do not replace a table with only charts when the report includes a table
        - do not remove caveat text for readability
        - do not compress multiple sections into one slide unless the current section is genuinely tiny

        CLIENT-SPECIFIC RULES

        - Olympic Holidays is a concise, data-led QBR
        - keep the deck focused on automated sections only
        - do NOT invent sections like competitor creative analysis, opportunities, Q&A, glossary, device analysis, or geo analysis unless they are explicitly present in `report.txt`
        - preserve labels like `YoY Matched-Month Comparison` exactly if they appear
        """
    ).strip(),
    "wightlink": dedent(
        """
        ...
        Your job is to convert this Wightlink QBR into a polished presentation.
        ...
        - preserve ALL bullet wording exactly unless only tiny grammar cleanup is needed
        - create a slide for EVERY section in the report
        - never invent sections not present in the report
        - never reintroduce removed sections like SEO, tests, audience insights, opportunities, or competitor analysis unless they are explicitly present in `report.txt`

        SOURCE PRIORITY

        1. `report.txt` is the source of truth
        2. the uploaded `.pptx` is the structure reference
        3. the example layout file is for visual style only

        STRUCTURE TO FOLLOW

        Expected Wightlink section types:

        1. Cover
        Create 1 title slide:
        - title: `Wightlink QBR`
        - preserve subtitle exactly

        2. Agenda
        Create 1 agenda slide:
        - preserve the agenda bullets exactly

        3. Trends
        Create 1 trends slide:
        - preserve exact title and subtitle
        - if a chart exists in the current PPT, redesign it professionally
        - if the section contains only fallback bullets, keep them and do NOT invent trend data

        4. Auction Insights - Generic
        Create 1 slide:
        - preserve the full table exactly
        - preserve all bullets exactly

        5. Auction Insights - Brand
        Create 1 slide:
        - preserve the full table exactly
        - preserve all bullets exactly
        ...
        7. All Performance YoY
        Create 1 slide:
        - preserve the YoY table exactly
        - preserve all bullets exactly

        8. Brand Performance
        Create 1 slide:
        - create visuals for the brand monthly trend
        - preserve all bullets exactly

        9. Brand Summary
        Create 1 slide:
        - preserve the summary table exactly
        - preserve all bullets exactly

        10. Generics Performance
        Create 1 slide:
        - create visuals for the generic monthly trend
        - preserve all bullets exactly
        ...
        - preserve all bullets exactly
        - create visuals only if they can be derived without altering the content

        13. Closing
        Create 1 closing slide:
        - preserve the closing message exactly

        VISUAL RULES

        - time series -> line chart
        - side-by-side metric comparison -> bar chart
        - do not remove data tables where they already exist
        - keep section titles exactly as written
        - do not merge brand and generic into one slide
        - do not invent extra strategic commentary

        CLIENT-SPECIFIC RULES

        - Wightlink is a tightly scoped QBR
        - keep it clean and business-like
        - preserve the exact structure that exists in the report
        - do NOT add removed legacy sections unless they explicitly appear in `report.txt`
        """
    ).strip(),
    "wendy_wu": dedent(
        """
        ...
        Your job is to convert this Wendy Wu Tours PPC report into a polished presentation.
        ...
        - preserve ALL bullet wording exactly unless only tiny grammar cleanup is needed
        - create a slide for EVERY section in the report
        - convert EVERY dataset into a visual
        - never omit a campaign block or destination block that exists in the report
        ...
        This Wendy Wu report follows repeating patterns.

        TYPE 1 — OVERALL PERFORMANCE
        ...
        Create:
        - 1 competitive landscape slide
        - preserve the table exactly
        - preserve bullets exactly

        TYPE 7 — RECOMMENDATIONS / NEXT STEPS
        If present:
        - create 1 recommendations slide
        - preserve bullets exactly

        CHART RULES

        - every time series -> line chart
        - every distribution -> pie or donut chart
        ...
        - Wendy Wu is the most structurally dense client
    ...
    "wendy_wu_australia": dedent(
        ...
        Your job is to convert this Wendy Wu Tours Australia PPC report into a polished presentation.
        ...
        This report follows the same structural pattern as Wendy Wu Tours.
        ...
        - This is Wendy Wu Tours Australia, so keep the client name exact throughout
