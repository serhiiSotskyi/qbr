...
CLIENT_SOURCE_RULES: dict[str, dict[str, Any]] = {
    ...
    "wendy_wu_australia": {
        ...
        "currency": "AUD",
        "ga4_property_env": (
            "GA4_PROPERTY_ID_WENDY_WU_AUSTRALIA",
            "GA4_PROPERTY_ID_WWT_AUS",
        ),
    ...
    "olympic_holidays": {
        ...
        "currency": "GBP",
        "ga4_property_env": (
            "GA4_PROPERTY_ID_OLYMPIC_HOLIDAYS",
            "GA4_PROPERTY_ID_OLYMPIC",
        ),
...
def ga4_source_status(client_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "message": (
            auth_status["message"]
            if property_id
            else f"Missing GA4 property ID for {client_id}."
        ),
    }
...
def dataforseo_source_status() -> dict[str, Any]:
    ...
    return {
        ...
        "message": (
            ...
            else "Missing DataForSEO login or password."
        ),
    }
...
def client_has_trends(client_config: dict[str, Any], report_mode: str) -> bool:
    ...
    destination_enabled = bool(
        client_config.get("destination_trends", {}).get("enabled")
    )
...
def prepare_automated_source_inputs(
    *,
    ...
    if (
        use_dataforseo_trends
        and report_mode == "quarterly"
        and client_has_trends(client_config, report_mode)
    ):
        if not performance_csv_path:
            raise AutomatedSourceError(
                "DataForSEO trends need a performance CSV first so the report quarter can be detected."
            )
        period = detect_period_from_performance(
            client_id, performance_csv_path, report_mode
        )
    ...
    if performance_csv_path:
        manifest_period = detect_period_from_performance(
            client_id, performance_csv_path, report_mode
        )
...
def generate_wightlink_plan_sheet_csv(
    *,
    ...
    try:
        ...
        Exception
    ) as exc:  # noqa: BLE001 - returned to Streamlit as a clear fallback warning
        raise AutomatedSourceError(
    ...
    manifest_path = (
        ...
        else request_path / "source_data" / "SOURCE_GENERATION_MANIFEST.json"
    )
...
def _build_csv_freshness_validation(
    *,
    ...
    if "Date" not in normalized.columns:
        raise AutomatedSourceError(
            "Generated performance CSV is missing a Date column."
        )
    ...
    period_rows = working[
        (working["_date"] >= period.start) & (working["_date"] <= period.end)
    ].copy()
    present_days = sorted(
        period_rows["_date"].dt.strftime("%Y-%m-%d").dropna().unique().tolist()
    )
    expected_days = (
        pd.date_range(period.start, period.end, freq="D").strftime("%Y-%m-%d").tolist()
    )
    ...
    if period_rows.empty:
        errors.append(
            f"generated CSV has no rows for expected {period.label} period {_fmt_date(period.start)} to {_fmt_date(period.end)}"
        )
    else:
    ...
    return {
        ...
        "period_date_min": (
            ...
            if date_min is not None and not pd.isna(date_min)
            else None
        ),
        "period_date_max": (
            date_max.strftime("%Y-%m-%d")
            if date_max is not None and not pd.isna(date_max)
            else None
        ),
...
def _sum_activity_columns(df: pd.DataFrame, client_id: str) -> float:
    ...
    for column in metric_columns:
        if column in df.columns:
            total += (
                pd.to_numeric(df[column].map(_to_float), errors="coerce")
                .fillna(0.0)
                .sum()
            )
    return float(total)
...
def _summarize_performance_totals(df: pd.DataFrame, client_id: str) -> dict[str, float]:
    ...
    for key, column in metric_map.items():
        if column not in df.columns:
            ...
            continue
        totals[key] = float(
            pd.to_numeric(df[column].map(_to_float), errors="coerce").fillna(0.0).sum()
        )
    return totals
...
def _fetch_wightlink_direct_ga4_totals(
    *,
    ...
    totals = {
        ...
        "clicks": sum(
            _to_float(row.get("advertiserAdClicks")) or 0.0 for row in cost_rows
        ),
        "impressions": sum(
            _to_float(row.get("advertiserAdImpressions")) or 0.0 for row in cost_rows
        ),
        "purchases": sum(_to_float(row.get("keyEvents")) or 0.0 for row in event_rows),
        "purchase_revenue": sum(
            _to_float(row.get("purchaseRevenue")) or 0.0 for row in event_rows
        ),
    }
    return {
        "property_id": property_id,
...
def _compare_source_totals_to_direct_ga4(
    source_totals: Mapping[str, Any], direct_totals: Mapping[str, Any]
) -> dict[str, Any]:
    errors: list[str] = []
...
def generate_dataforseo_trend_csvs(
    *,
    ...
    location_name = str(
        rules.get("trend_location")
        or _trend_location_from_country(client_config.get("country"))
    )
    request_path = Path(output_dir)
    source_data_dir = request_path / "source_data"
    ...
    return AutomatedSourcePaths(
        trends_dir=(
            olympic_trends_dir if any(olympic_trends_dir.glob("*.csv")) else None
        ),
        trends_ytd_current_dir=current_dir if any(current_dir.glob("*.csv")) else None,
        trends_ytd_previous_dir=(
            previous_dir if any(previous_dir.glob("*.csv")) else None
        ),
        source_data_dir=source_data_dir,
...
def detect_period_from_performance(
    client_id: str, performance_csv: str | Path, report_mode: str
) -> SourcePeriod:
    if client_id == "wightlink":
        ...
        return SourcePeriod(
            "quarterly", quarter.year, quarter.quarter, quarter.start, quarter.end
        )
    ...
    if client_id == "olympic_holidays":
        ...
        if df.empty:
            raise ValueError(
                "Olympic Holidays generated performance CSV has no valid dates."
            )
...
def default_source_period(
    report_mode: str, today: pd.Timestamp | None = None
) -> SourcePeriod:
    today_ts = (
    ...
    if report_mode == "monthly":
        ...
        return SourcePeriod(
            "monthly", int(month_start.year), None, month_start, month_end
        )

    if report_mode == "annual":
        year = int(today_ts.year) - 1
        return SourcePeriod(
            "annual", year, None, pd.Timestamp(year, 1, 1), pd.Timestamp(year, 12, 31)
        )

    current_quarter_start_month = ((int(today_ts.month) - 1) // 3) * 3 + 1
    latest_complete_end = pd.Timestamp(
        today_ts.year, current_quarter_start_month, 1
    ) - pd.Timedelta(days=1)
    quarter = int((latest_complete_end.month - 1) // 3) + 1
    return SourcePeriod(
        "quarterly",
        int(latest_complete_end.year),
        quarter,
        pd.Timestamp(latest_complete_end.year, (quarter - 1) * 3 + 1, 1),
        latest_complete_end,
    )
...
def resolve_trend_terms(client_config: dict[str, Any]) -> list[str]:
    ...
    if brand_config.get("enabled"):
        terms.extend(
            str(term).strip()
            for term in brand_config.get("terms", [])
            if str(term).strip()
        )
    ...
    if destination_config.get("enabled"):
        for destination in destination_config.get("destinations", []):
            terms.extend(
                str(term).strip()
                for term in destination.get("terms", [])
                if str(term).strip()
            )
...
def dataforseo_response_to_frame(
    response: dict[str, Any], fallback_keyword: str
) -> pd.DataFrame:
            ...
            for item in result.get("items") or []:
                ...
                for point in item.get("data") or []:
                    point_date = pd.to_datetime(
                        point.get("date_from") or point.get("date_to"), errors="coerce"
                    )
                    if pd.isna(point_date):
                        continue
                    values = _coerce_trend_values(
                        point.get("values"), item_keywords, fallback_keyword
                    )
...
def write_split_trend_csvs(
    *,
    ...
    previous = _trend_window(
        trend_df, period.previous_ytd_start, period.previous_ytd_end
    )
...
def write_source_generation_manifest(
    *,
    ...
    generated_files = {
        ...
        "trends_ytd_previous_dir": _relative_path(
            trends_ytd_previous_dir, request_path
        ),
    ...
    manifest = {
        "source_generation": {
            ...
            "ga4": {
                ...
                "raw_json_dir": _relative_path(
                    request_path / "raw_api" / "ga4", request_path
                ),
            ...
            "dataforseo": {
                ...
                "location_name": (
                    str(_client_rules(client_id).get("trend_location", ""))
                    if client_id in CLIENT_SOURCE_RULES
                    else ""
                ),
                "terms": (
                    resolve_trend_terms(client_config) if use_dataforseo_trends else []
                ),
                "raw_json_dir": _relative_path(
                    request_path / "raw_api" / "dataforseo", request_path
                ),
...
def validate_generated_performance_against_fixture(
    *,
    ...
    generated = _filter_validation_period(
        generated, start_date=start_date, end_date=end_date
    )
    fixture = _filter_validation_period(
        fixture, start_date=start_date, end_date=end_date
    )
    group_columns, metrics = _validation_columns(client_id)
    generated_grouped = _validation_group(
        generated, group_columns, metrics, suffix="generated"
    )
    fixture_grouped = _validation_group(
        fixture, group_columns, metrics, suffix="fixture"
    )
    merged = generated_grouped.merge(
        fixture_grouped, on=group_columns, how="outer"
    ).fillna(0)
    for metric in metrics:
        merged[f"{metric} Delta"] = (
            merged[f"{metric} Generated"] - merged[f"{metric} Fixture"]
        )
    return merged.sort_values(group_columns).reset_index(drop=True)


def summarize_validation_deltas(
    validation_df: pd.DataFrame, *, client_id: str
) -> pd.DataFrame:
...
class GA4DataApiClient:
    def __init__(
        self, access_token: str | None = None, session: requests.Session | None = None
    ) -> None:
        ...
        *,
        ...
        while True:
            payload: dict[str, Any] = {
                "dateRanges": [
                    {"startDate": _fmt_date(start_date), "endDate": _fmt_date(end_date)}
                ],
            ...
            if response.status_code >= 400:
                raise AutomatedSourceError(
                    f"GA4 runReport failed for property {property_id}: {response.status_code} {response.text[:500]}"
                )
            data = response.json()
    ...
    def access_token(self) -> str:
        ...
        env_token = os.getenv("GA4_OAUTH_ACCESS_TOKEN") or os.getenv(
            "GOOGLE_API_OAUTH_ACCESS_TOKEN"
        )
        if env_token:
            self._access_token = env_token
            return self._access_token
        refresh_token = os.getenv("GA4_OAUTH_REFRESH_TOKEN") or os.getenv(
            "GOOGLE_API_OAUTH_REFRESH_TOKEN"
        )
        if refresh_token:
            self._access_token = _oauth_refresh_access_token(
                refresh_token, session=self.session
            )
            return self._access_token
...
class DataForSEOTrendsClient:
    ...
    def fetch_interest_over_time(
        ...
        *,
        ...
        if response.status_code >= 400:
            raise AutomatedSourceError(
                f"DataForSEO Trends request failed: {response.status_code} {response.text[:500]}"
            )
        data = response.json()
        if data.get("status_code") not in (20000, None):
            raise AutomatedSourceError(
                f"DataForSEO Trends returned {data.get('status_code')}: {data.get('status_message')}"
            )
        ...
        if task_errors:
            ...
            raise AutomatedSourceError(
                f"DataForSEO Trends task failed: {'; '.join(messages) or 'unknown task error'}"
            )
        return data
...
def _cost_rows_to_frame(
    rows: Sequence[dict[str, Any]], rules: Mapping[str, Any] | None = None
) -> pd.DataFrame:
    records = []
    for row in rows:
    ...
    if not records:
        return pd.DataFrame(
            columns=["date", "campaign_name", "cost", "clicks", "impressions"]
        )
    frame = pd.DataFrame(records).dropna(subset=["date"])
    return (
        frame.groupby(["date", "campaign_name"], as_index=False)[
...
def _event_rows_to_frame(
    rows: Sequence[dict[str, Any]], rules: dict[str, Any]
) -> pd.DataFrame:
    records = []
    for row in rows:
        date = _parse_ga4_date(row.get("date"))
        ...
        if _exclude_event_row(
            date, event_name, campaign, rules, channel_group=channel_group
        ):
            continue
        records.append(
            {
    ...
    frame = pd.DataFrame(records).dropna(subset=["date"])
    return (
        frame.groupby(["date", "campaign_name", "event_name"], as_index=False)[
            ["key_events", "purchase_revenue"]
        ]
        .sum(min_count=1)
        .reset_index(drop=True)
    )
...
def _merge_ga4_frames(cost_df: pd.DataFrame, event_df: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = (
        cost_df.merge(event_pivot, on=["date", "campaign_name"], how="outer")
        if not cost_df.empty
        else event_pivot
    )
    if merged.empty:
        raise AutomatedSourceError(
            "GA4 returned no usable paid performance rows for the requested period."
        )
    merged["date"] = _coerce_date_column(merged["date"])
    merged = merged.dropna(subset=["date"])
    if merged.empty:
        raise AutomatedSourceError(
            "GA4 returned no usable dated paid performance rows for the requested period."
        )
    for column in [
        "cost",
        "clicks",
...
def _pivot_event_metrics(event_df: pd.DataFrame) -> pd.DataFrame:
    ...
    working["purchases"] = working.apply(
        lambda row: row["key_events"] if str(row["event_name"]) == "purchase" else 0.0,
        axis=1,
    )
    working["add_to_cart"] = working.apply(
        lambda row: (
            row["key_events"] if str(row["event_name"]) == "add_to_cart" else 0.0
        ),
        axis=1,
    )
    return (
        working.groupby(["date", "campaign_name"], as_index=False)[
            ["sales_leads", "purchases", "add_to_cart", "purchase_revenue"]
        ]
        .sum(min_count=1)
        .reset_index(drop=True)
    )
...
def _build_olympic_performance_frame(
    ...
    *,
    ...
    output = (
        working.groupby(["Date", "Campaign Type"], as_index=False)[
            ["purchases", "purchase_revenue", "cost", "add_to_cart"]
        ]
        .sum(min_count=1)
        .rename(
            columns={
    ...
    output["Cost per ATC"] = output.apply(
        lambda row: _safe_div(row["Cost"], row["Add to cart"]), axis=1
    )
    output["AOV"] = output.apply(
...
def _build_ga4_other_campaign_source_frame(merged: pd.DataFrame) -> pd.DataFrame:
    output = (
        merged.groupby("campaign_name", as_index=False)[
            ["clicks", "impressions", "cost", "sales_leads"]
        ]
        .sum(min_count=1)
        .rename(
            columns={
    ...
    output["Source"] = "GA4"
    return output[
        ["Campaign", "Source", "Clicks", "Impressions", "Cost", "Conversions"]
    ]
...
def classify_campaign_type(campaign_name: Any) -> str:
    ...
    if not normalized or normalized == "not set":
        return "Other"
    if (
        "demand gen" in normalized
        or "demandgen" in compact
        or "discovery" in normalized
    ):
        return "Demand Gen"
    if (
        "performance max" in normalized
        or "performancemax" in compact
        or "pmax" in compact
    ):
    ...
    if (
        ...
        or "generics" in normalized
        or "non brand" in normalized
        or "nonbrand" in compact
    ):
        return "Generic"
    return "Other"
...
def _oauth_refresh_access_token(
    refresh_token: str, *, session: requests.Session | None = None
) -> str:
    ...
    token_url = (
        os.getenv("GA4_OAUTH_TOKEN_URL")
        or os.getenv("GOOGLE_API_OAUTH_TOKEN_URL")
        or "https://oauth2.googleapis.com/token"
    )
    if not client_id or not client_secret:
        raise AutomatedSourceError(
            "GA4 refresh-token auth requires OAuth client ID and client secret env values."
        )
    ...
    if response.status_code >= 400:
        raise AutomatedSourceError(
            f"GA4 OAuth refresh failed: {response.status_code} {response.text[:300]}"
        )
...
def _ga4_auth_status() -> dict[str, Any]:
    if os.getenv("GA4_OAUTH_ACCESS_TOKEN") or os.getenv(
        "GOOGLE_API_OAUTH_ACCESS_TOKEN"
    ):
        return {
            "configured": True,
            "method": "oauth_access_token",
            "message": "GA4 access token configured for quick testing.",
        }
    if os.getenv("GA4_OAUTH_REFRESH_TOKEN") or os.getenv(
        "GOOGLE_API_OAUTH_REFRESH_TOKEN"
    ):
        client_id = _first_env(
            "GA4_OAUTH_CLIENT_ID",
        ...
        if client_id and client_secret:
            return {
                ...
                "message": "GA4 OAuth refresh token configured.",
            }
        return {
            "configured": False,
    ...
    if (
        ...
        or os.getenv("GA4_SERVICE_ACCOUNT_JSON_PATH")
        or os.getenv("GA4_SERVICE_ACCOUNT_JSON_BASE64")
    ):
        return {
            "configured": True,
            "method": "service_account",
            "message": "GA4 service-account auth configured.",
        }
    return {
        "configured": False,
        "method": "none",
...
def _service_account_access_token() -> str:
    ...
    credentials_path = os.getenv("GA4_SERVICE_ACCOUNT_JSON_PATH") or os.getenv(
...
def _resolve_ga4_property_id(client_id: str, *, required: bool = True) -> str | None:
    ...
    if required:
        expected = ", ".join(
            rules.get("ga4_property_env", (f"GA4_PROPERTY_ID_{client_id.upper()}",))
        )
        raise AutomatedSourceError(
            f"Missing GA4 property ID for {client_id}. Set one of: {expected}."
        )
    return None
...
def _client_rules(client_id: str) -> dict[str, Any]:
    ...
    if not rules:
        raise AutomatedSourceError(
            f"Automated GA4/DataForSEO source generation is not configured for client '{client_id}'."
        )
    return rules
...
def _dataforseo_login(*, required: bool = True) -> str | None:
    ...
    if required:
        raise AutomatedSourceError(
            "Missing DataForSEO login. Set DATAFORSEO_LOGIN or DATAFORSEO_USERNAME."
        )
    return None
...
def _dataforseo_password(*, required: bool = True) -> str | None:
    ...
    if required:
        raise AutomatedSourceError(
            "Missing DataForSEO password. Set DATAFORSEO_PASSWORD."
        )
    return None
...
def _exclude_event_row(
    ...
    *,
    ...
    for exclusion in rules.get("exclude_event_channel_campaign_exact", ()):
        if (
            not isinstance(exclusion, Sequence)
            or isinstance(exclusion, str)
            or len(exclusion) < 2
        ):
            continue
        if normalized_channel == _normalize_text(
            exclusion[0]
        ) and normalized_campaign_exact == _normalize_text(exclusion[1]):
            return True
    ...
    if date is not None and not pd.isna(date):
        ...
        allowed_not_set_events = {
            str(value).strip()
            for value in rules.get("allow_current_year_not_set_events", ())
        }
        if (
            exclude_from
...
def _exclude_cost_row(
    date: pd.Timestamp | None, campaign_name: str, rules: Mapping[str, Any]
) -> bool:
...
def _trend_window(
    trend_df: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp
) -> pd.DataFrame:
...
def _write_google_trends_csv(
    path: str | Path, trend_df: pd.DataFrame, term: str
) -> None:
    output = Path(path)
    ...
    working = trend_df[
        trend_df["term"].map(
            lambda value: _normalize_text(value) == _normalize_text(term)
        )
    ].copy()
    if working.empty:
        working = trend_df.copy()
    working = working.sort_values("date").rename(
        columns={"date": "Week", "value": term}
    )
...
def _coerce_trend_values(
    values: Any, keywords: Sequence[str], fallback_keyword: str
    ...
    if isinstance(values, list):
        ...
        for index, value in enumerate(values):
            keyword = (
                keyword_list[index] if index < len(keyword_list) else keyword_list[0]
            )
            if isinstance(value, dict):
                output.append(
                    (
                        str(value.get("keyword") or keyword),
                        _to_float(value.get("value")),
                    )
                )
            else:
...
def _validation_columns(client_id: str) -> tuple[list[str], list[str]]:
    if client_id in {"wendy_wu", "wendy_wu_australia", "wendy_wu_uk"}:
        return ["Date", "Campaign Type", "Destination"], [
            "Cost",
            "Sales Leads",
            "Revenue",
            "Clicks",
    ...
    if client_id == "wightlink":
        return ["Date", "Campaign Type", "Data Type"], [
            "Cost",
            "Purchases",
            "Purchase Revenue",
            "Clicks",
...
def _validation_group(
    df: pd.DataFrame, group_columns: list[str], metrics: list[str], *, suffix: str
) -> pd.DataFrame:
    working = df.copy()
    for column in group_columns:
        if column == "Date":
            working[column] = _coerce_date_column(working[column]).dt.strftime(
                "%Y-%m-%d"
            )
        else:
    ...
    return grouped.rename(
        columns={metric: f"{metric} {suffix.title()}" for metric in metrics}
    )
